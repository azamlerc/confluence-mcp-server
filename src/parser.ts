...
export interface ConfluencePoint {
  lat: number;
  lon: number;
  prefix: string;           // e.g. "57°N 24°E"
  country: string;          // e.g. "Latvia"
  region?: string;          // e.g. "Euskadi" (not always present)
  locationDescription: string; // e.g. "4.8 km (3.0 miles) SW of Bolderāja, Rīgas, Latvia"
  distanceKm: number;       // numeric distance to nearest named place
  direction: string;        // e.g. "SW"
  nearestPlace: string;     // e.g. "Bolderāja"
  altitudeM?: number;       // e.g. 19
  visitCount: number;       // e.g. 12
 ...
 */
function parseLocationDescription(desc: string): {
  distanceKm: number;
  direction: string;
  nearestPlace: string;
} | null {
 ...
 */
export async function fetchConfluencePoint(
  lat: number,
  lon: number,
  timeoutMs = 30000
): Promise<ConfluencePoint | ConfluenceError> {
 ...
 */
export function parseConfluenceHtml(
  html: string,
  lat: number,
  lon: number
): ConfluencePoint | ConfluenceError {
  ...
  for (const h3 of h3s) {
    ...
    if (/\d+\.\d+\s*km/.test(text)) {
      ...
      if (altMatch) altitudeM = parseInt(altMatch[1]);
      break;
    }
  }
